...
1. Canonicalises `DATA_DIR` (`realpath -m`, so it works before the directory exists) and refuses to start unless the resolved path is `/config` itself or a path below it — this catches equivalent forms like `//`, `/./`, or `/config/../..` that resolve elsewhere, not just an exact string match. `DATA_DIR` is a user-configurable env var that's about to be recursively `chown`ed as root, so it's scoped to the one directory this image actually promises the repair for (`/config`, per [Persistent Data](#persistent-data) above) — a blocklist of specific dangerous values (`/`, `/app`, ...) would still let others (`/etc`, `/usr`, ...) through.
