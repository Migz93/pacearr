...
3. Skips straight to step 4 if a marker file, `/config/.docker-entrypoint-ownership-ok`, already exists and is `node`-owned. The marker — not `/config`'s own top-level ownership — is the completion signal: GNU `chown -R` sets ownership on the directory argument itself before recursing into its children, so if the walk were ever interrupted or a nested `chown` failed partway through, `/config`'s top level could already look `node`-owned even though children (database, cache, logs) are still root-owned and unwritable. The marker is only written after the recursive repair below has fully succeeded, so the fast path can only ever trigger once a complete repair has actually finished. Otherwise runs `chown -R -P -h node:node "$DATA_DIR"`, then writes the marker. The `-R` walk uses coreutils' own fd-relative traversal rather than re-resolving path strings, so a directory swapped out from under the walk can't redirect it outside `DATA_DIR`. The `-h` flag makes `chown` act on symlinks themselves rather than following them, so a symlink under `/config` pointing outside it gets re-owned itself without touching whatever it points to. `-P` (GNU's default for `-R`, spelled out explicitly here) means a symlinked directory is never traversed into, so `-h`'s protection can't be defeated by nesting the escape one level deeper via a symlinked subdirectory. This fast path only checks the marker, not every nested entry, so a file dropped into `/config` out-of-band as root after a prior successful repair (e.g. a manual host-side copy) won't be caught until the marker or its ownership is reset — that surfaces as an app-level write failure on that one file, not a security gap, since skipping the repair never causes anything to be touched that otherwise wouldn't have been.
