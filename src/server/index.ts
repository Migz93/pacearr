...
// Every job needs Plex and Sonarr configured. The Plex and Sonarr settings
// routes call resumeAfterSetup() once both are saved.
scheduler.setReadiness(isReady);
...
scheduler.registerRecurringJob({
  ...
  task: () => services.importHistory().then(() => undefined),
});
scheduler.registerRecurringJob({
  ...
  task: () => services.checkTautulliActiveSessions().then(() => undefined),
});
scheduler.registerRecurringJob({
  ...
  task: () => services.reconcileFullHistory().then(() => undefined),
});
scheduler.registerRecurringJob({
  id: "rolling-reconcile",
  intervalMs: settings.rollingReconcileIntervalHours * 60 * 60 * 1000,
  task: () => services.reconcileRollingShows().then(() => undefined),
});
scheduler.registerRecurringJob({
...
scheduler.registerRecurringJob({
  ...
  task: () => services.refreshRecommendations(),
});
scheduler.registerRecurringJob({
  id: "new-show-triage",
