...
| History import scopes identity cache entries to the configured source connection | A reused Tautulli rating key is resolved again after its configured server/API key changes, rather than reusing metadata from the prior instance |
...
| Enrollment repairs and seeds previously unmatched history with a full verified source read | Enrolling after a source event was stored without a verified series link re-reads source history, repairs the exact event, and seeds viewer progress immediately |
