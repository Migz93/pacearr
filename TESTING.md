...
| History import reads a changed Tautulli server in full instead of resuming from the old server's cursor | A watch older than the stored cursor is imported when the cursor belongs to a different server; the same server, or state saved before connections were recorded, still resumes incrementally |
