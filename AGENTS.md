...
| Test suite | Server tests — `npm test` and `npm run test:server`. No Playwright suite. |
...
`pacearr-frontend`, `pacearr-api` etc.
...
Map `/opt/pacearr` on the host to `/config` inside the container (the app
...
Your workspace is mounted at `/workspaces/pacearr` inside this container. This
is the same directory as `app/` on the host at `/opt/vscode/node/pacearr/app/`.
...
Pacearr runs on port **9302**. Always map `9302:9302` — do not
...
Standard rebuild sequence (run from `/workspaces/pacearr`):
...
You should see: `Pacearr listening on port 9302`.
...
Pacearr is worked on by two AI agents — Claude and Codex — usually in separate
...
- Call out explicitly if the change affects: release behaviour, Docker
  publishing, auth, database schema, Plex/Sonarr/Tautulli integrations, cleanup/deletion behaviour, or user-visible
  setup
